Public Class UserGroupDataFactory
    ...
    Public Function GetByUserId(settings As ISettings, ByVal organizationId As Guid, ByVal userId As Guid) As IEnumerable(Of UserGroupData) Implements IUserGroupDataFactory.GetByUserId
        Return Me.GetByUserId(settings, New DbProviderFactory(), organizationId, userId)
    End Function

    Public Function GetByUserId(settings As ISettings, ByVal providerFactory As IDbProviderFactory, ByVal organizationId As Guid, userId As Guid) As IEnumerable(Of UserGroupData)
        ...
        Using connection As IDbConnection = providerFactory.OpenConnection(settings.ConnectionString)
            Using command As IDbCommand = connection.CreateCommand
                command.CommandText = "clnt.sUserGroupByUserId"
                command.CommandType = CommandType.StoredProcedure
                parameter = CreateParameter(providerFactory, "userId", DbType.Guid)
                parameter.Value = userId
                command.Parameters.Add(parameter)

                parameter = CreateParameter(providerFactory, "organizationId", DbType.Guid)
