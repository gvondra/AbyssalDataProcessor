<TestClass()>
Public Class UserGroupSaverTest

    <TestMethod()>
    Public Sub CreateTest()
        Dim transactionHandler As New Mock(Of ITransactionHandler)()
        Dim data As New UserGroupData()
        Dim saver As New UserGroupDataSaver(transactionHandler.Object, data)
        ...
        command.VerifySet(Sub(c As IDbCommand) c.CommandText = "clnt.iUserGroup", Times.AtLeastOnce)
    End Sub
    ...
    Public Sub UpdateTest()
        ...
        command.Verify(Of Integer)(Function(c As IDbCommand) c.ExecuteNonQuery(), Times.Once)
        command.VerifySet(Sub(c As IDbCommand) c.CommandType = CommandType.StoredProcedure, Times.AtLeastOnce)
        command.VerifySet(Sub(c As IDbCommand) c.CommandText = "clnt.uUserGroup", Times.AtLeastOnce)
    End Sub
