<TestClass()>
Public Class OrganizationDataSaverTest

    <TestMethod()>
    Public Sub CreateTest()
        Dim transactionHandler As New Mock(Of ITransactionHandler)()
        Dim data As New OrganizationData()
        Dim saver As New OrganizationDataSaver(transactionHandler.Object, data)
        ...
        command.VerifySet(Sub(c As IDbCommand) c.CommandText = "adp.iOrganization", Times.AtLeastOnce)
    End Sub
    ...
    Public Sub UpdateTest()
        ...
        command.Verify(Of Integer)(Function(c As IDbCommand) c.ExecuteNonQuery(), Times.Once)
        command.VerifySet(Sub(c As IDbCommand) c.CommandType = CommandType.StoredProcedure, Times.AtLeastOnce)
        command.VerifySet(Sub(c As IDbCommand) c.CommandText = "adp.uOrganization", Times.AtLeastOnce)
    End Sub
